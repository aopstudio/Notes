...
设函数z=f(x,y)在点$(x_0,y_0)$的某邻域内有连续的二阶偏导数，且$f'_x(x_0,y_0)=0,f'_y(x_0,y_0)=0$，令$f''_{xx}(x_0,y_0)=A,f''_{xy}(x_0,y_0)=B,f''_{yy}(y_0,y_0)=C$，则
...
#### 3. 任意项级数（$\sum^\infty_{n=1}u_n,u_n为任意实数$）
1. 绝对收敛与条件收敛的概念
若级数$\sum^\infty_{n=1}|u_n|$收敛，则称级数$\sum^\infty_{n=1}u_n$**绝对收敛**  
若级数$\sum^\infty_{n=1}u_n$收敛，而级数$\sum^\infty_{n=1}|u_n|$发散，则称级数$\sum^\infty_{n=1}u_n$条件收敛

2. 关于绝对收敛和条件收敛的基本结论
        1. 绝对收敛的级数一定收敛，即$\sum^\infty_{n=1}|u_n|收敛\Rightarrow \sum^\infty_{n=1}u_n收敛$
        2. 条件收敛的级数所有正项（或负项）构成的级数一定发散
        即：若级数$\sum^\infty_{n=1}u_n$条件收敛，则$\sum^\infty_{n=1}\frac{u_n+|u_n|}{2}$和$\sum^\infty_{n=1}\frac{u_n-|u_n|}{2}$都发散
